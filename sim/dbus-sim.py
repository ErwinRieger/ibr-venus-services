...
class Simulation:
    ...
    def __init__(self, scale, installed_pv, steps, delay, services, 
        ...
        logger.info(f"interval: {self.interval} seconds, {self.interval/3600} h")
        self.step = 0
    ...
    async def loop(self):
        ...
        while self.step < self.steps+1:
            ...
            logger.info(f"simul loop {self.step}/{self.steps}")
            ...
            for service in self.services:
                with service as s:
                    ...
                    # logger.info("simul loop, step, random:", self.step, rand);
        ...
        if self.twait > 0:
            logger.info(f"loop done, waiting {self.twait}");
...
def main():
    ...
    parser.add_argument('--dbus', help='dbus bus to use, defaults to system',
            default='system')
    parser.add_argument('--twait', help='Wait seconds after simulation (-1 for repeat).',
            default=0)
    parser.add_argument('--steps', help='Numbers of steps to run.',
            default=24)
    parser.add_argument('--delay', help='Delay between steps [s].',
            default=0.5)
    parser.add_argument('--charger', help='Charger Config.')
